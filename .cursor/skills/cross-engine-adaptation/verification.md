...
**L7 用 4096 环境跑，不用小规模。** 这是生产规模。两个引擎在这里承压的方式不同，别混为一谈：

- **Isaac 是总量。** `gpu_collision_stack_size` / `gpu_max_rigid_patch_count` 是全局缓冲，需求随环境数线性涨——main 把碰撞栈设成 `2**29` 正是因为它按生产规模跑。几百个环境根本压不到，溢出守卫也就从头到尾没被调用过。
- **mjlab 是尾部采样。** `nconmax` / `njmax` 是**每 world** 的分配（预算是 `nconmax * nworld`），每 world 的余量不随环境数变化。规模在这里买到的不是更紧的预算，而是更多样本：某个罕见的高接触位形在 256 个 world 上可能一次都不出现，在 4096 个上几乎必然出现。

两条都指向同一个结论——小规模跑绿只说明小规模没问题，而溢出的后果是**静默丢接触**，正是 L7 该抓的那一类。`scripts/train.py` 的 `--num_envs` 默认就是 4096，对拍时不要覆盖它。

同一层还有个采样问题：**单个种子分不开信号和噪声**。回合长度 0.93 与 1.02 这种量级的差异，在一个种子上无法判断是实现漂移还是随机性。要下「这是一处漂移」的结论，要么差异大到离谱（如 `dof_acc_l2` 的 1.71 倍），要么多跑几个种子。
