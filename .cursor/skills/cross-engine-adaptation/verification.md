...
**L7 用 4096 环境跑，不用小规模。** 这是生产规模，也是唯一能压到预算的规模。接触与约束预算随环境数增长：mjlab 的 `nconmax` / `njmax`、Isaac 的 `gpu_collision_stack_size` / `gpu_max_rigid_patch_count` 在几百个环境下都远没触顶，于是溢出守卫从头到尾没被调用过——而溢出的后果是**静默丢接触**，正是 L7 该抓的那一类。小规模跑绿只说明小规模没问题。`scripts/train.py` 的 `--num_envs` 默认就是 4096，对拍时不要覆盖它。
