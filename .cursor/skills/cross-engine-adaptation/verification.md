...
**别拿训练曲线去回答能直接量的问题。** 两次训练之间的差，混了策略质量、随机性和实现差异三样，拆不开；很多时候那个问题根本不需要训练。`track_lin_vel_xy_exp` 用 COM 还是连杆速度，看两次 run 的 `error_vel_xy` 是答不出来的——**两边各自量的是自己的参考系**，比值里分不清是跟得差还是量的东西不同。换成在同一次 rollout 里按两种参考系各打一次分（`scripts/probe_velocity_frame.py`），策略质量当场抵消，答案是奖励差 0.2%，四行漂移一次性定案。先问「这能不能在一次 rollout 里量出来」，再考虑排训练。
