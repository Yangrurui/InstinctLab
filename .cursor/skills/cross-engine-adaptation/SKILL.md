...
- **不要擅自停掉用户正在跑的训练。** 改代码、换 GPU、换日志名、提交后重跑都不是停训理由；目标 GPU 已被占用时先问用户。见 [training-process-safety.mdc](../../rules/training-process-safety.mdc)。
...
上面「两侧都验」这条早就在这份文档里，而漂移表依然整片只钉了自己那一侧：`assert compiled.env_cfg.sim.nconmax == 256`、`assert scanners[...].origin_offset == (0.04, 0, 20.0)`、地形配方里一串手抄的 `0.07 / 0.35 / 1.54`。参照那边改成一样的值，这些断言一条都不会红——记的是差异，测的却只有半边。所以补一句该落到哪：**规则要落到表上，不只落到读取器上**。读取器那一层扫过之后，表这一层还得再扫一遍。

两个具体的招式，光看「测试是绿的」都发现不了：

- **按名字断言等于没断言。** `assert rewards["track_lin_vel_xy_exp"].func.__name__ == "track_lin_vel_xy_exp"` 是三行 velocity_frame 漂移的全部内容。可那三行记的是**参考系**（COM 还是连杆原点），函数叫什么名字和参考系毫无关系——两侧同时改掉参考系，这断言纹丝不动。**断言要落在那一行声称的那个量上**，不是落在承载它的符号上。
- **名字里写着 “matches X” 却从没读过 X。** `test_terrain_recipe_constants_match_instinctmj` 断言的全是手抄字面量，一个字节都没读 InstinctMJ。这种测试比没有更糟：它给出的是「已核对」的印象，而核对发生在某次人工阅读的那一刻，之后再没发生过。**凡是名字里带参照的测试，必须真的去读参照**；读不到就抛，别退化成只比自己。
...
驱动器延迟的这一处，是本项目至今最纯粹的一次静默重划分：没有任何一处配置写错，只是**分组的键选错了**。

mjlab 的 `DelayBuffer` 会把「延迟设置相同」的 actuator 融成一个，所以决定「哪些关节抽同一个滞后」的是 `delay_update_period`，不是 actuator 的拆分方式。我们的 mjlab 适配层按 PD 增益四元组给关节分组，再拿组序号当 period。增益相同的关节确实可以共用一个 cfg——那一步是对的、纯粹是批处理优化——但拿它当**滞后的相关性结构**就换了个意思。结果：G1 一条腿的髋 pitch/yaw 和髋 roll/knee 落在两个抽样上（它们增益不同），踝和腰绑在一起（它们增益相同）。两个参照都不是这么分的，两边都是腿/踝/腰/腰yaw/臂。

一条腿内部可以「髋滞后 0 步、膝滞后 2 步」的机器人，和一条整体晚 2 步的机器人，是两个不同的被控对象。没有任何异常，训练照常收敛，只有 25–35% 更高的 `root_height` 摔倒率——而那个数字有太多别的解释。

几条：

- **物理事实要在引擎无关层声明一次，两个引擎各自去读它。** 这里是 `JointProperties.actuator_group`（关节挂在哪条电机总线上）。适配层不该发明分区，它只把声明映射到本引擎的机制上。Isaac 恰好一直是对的（它照抄了 main 的分组字典），只有 mjlab 用了推导——**一处推导一处照抄，是最难发现的组合**，因为两边都「有依据」。
- **别拿方便的键代替正确的键。** 增益和接线是两个不同的分区，在 G1 上它们交叉：`legs` 跨两套增益，`arms` 跨两套增益。任何时候用 A 的相等性去代表 B 的相等性，先问这两个分区在这台机器人上是不是真的重合。
- **「必须两两不同」这类断言会把旧意图焊死。** 原来有条测试写 `assert len(set(periods)) == len(periods)`，注释是「防止 mjlab 把组融到一起」。可融合正是共用滞后的唯一手段，腿需要它。断言该钉的是分区本身（几条总线、各有哪些关节），不是 period 的个数。
- **对拍要走真实的接线路径。** 新测试从 `_torque_delay_kwargs` 算分区，而不是直接读 `actuator_group`——后者是拿声明验声明，接线换成别的来源它照样绿。
