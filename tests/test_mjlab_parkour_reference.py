...
pytestmark = pytest.mark.skipif(
    not mj_ref.available(), reason="InstinctMJ is not checked out"
)
...
def task():
    ...
    return parkour_target_g1(robot_spec(registry.asset_id(task_id)))
...
def test_reward_sets_differ_only_by_the_documented_extra(task) -> None:
    ...
    assert [n for n in mj_ref.reward_names()] == [
        n for n in task.mdp.rewards["rewards"] if n != "dof_vel_limits"
    ]
...
def test_shared_reward_weights_match(task) -> None:
    ...
    shared = {
        ...
        if name != "volume_points_penetration"
    }
    assert shared == {
        name: weight
...
def test_shared_reward_numeric_params_match_effective_shoe(task) -> None:
    ...
    for name, keys in numeric_keys.items():
        for key in keys:
            assert declared[name].params.get(key) == reference[name].get(key), (
                f"{name}.{key}"
            )
    assert (
        declared["feet_at_plane"].params["height_offset"] == mj_ref.SHOE_HEIGHT_OFFSET
    )
...
def test_observation_order_noise_and_scale_match(task) -> None:
    for group in ("policy", "critic", "amp_policy", "amp_reference"):
        assert tuple(task.mdp.observations[group].terms) == mj_ref.observation_order(
            group
        ), group
...
def test_command_velocity_boxes_keep_reference_values_under_the_shared_mesh_box_name(
    task,
) -> None:
    ...
    assert (
        resolved["velocity_control_stiffness"]
        == reference["velocity_control_stiffness"]
    )
    assert (
        resolved["heading_control_stiffness"] == reference["heading_control_stiffness"]
    )
...
def test_sim_timings_match_and_solver_diffs_are_the_documented_ones(
    task, compiled
) -> None:
    ...
    assert (
        compiled.env_cfg.sim.contact_sensor_maxmatch
        == overrides["contact_sensor_maxmatch"]
        == 128
    )
    ...
    assert (
        compiled.env_cfg.sim.mujoco.ccd_iterations
        == overrides["ccd_iterations"]
        == mj_ref.CCD_ITERATIONS
    )
...
def test_parkour_robot_matches_instinctmj_on_spawn_delay_shoe(task, compiled) -> None:
    ...
    pd_actuators = (
        ...
        if type(act).__name__ == "BuiltinPdActuatorCfg"
    )
...
def test_compiled_scanners_volume_contact_and_camera_are_the_documented_drifts(
    task, compiled
) -> None:
    robot = compiled.env_cfg.scene.entities["robot"]
    assert robot.init_state.pos[2] == pytest.approx(mj_ref.SPAWN_Z)
    ...
    assert (
        reference_sensors["left_height_scanner"]["max_distance"]
        == mj_ref.SCANNER_MAX_DISTANCE
    )
    ...
    assert (
        reference_sensors["contact_forces"]["cfg_class"]
        == "ForceThresholdContactSensorCfg"
    )
    assert (
        contact.force_threshold
        == reference_sensors["contact_forces"]["force_threshold"]
        == 1.0
    )
...
def test_compiled_friction_is_the_union_instinctmj_states(compiled) -> None:
    ...
    assert "shared_random" not in params
...
def test_the_terrain_reader_would_notice_a_change_on_their_side() -> None:
    ...
    assert all(
        ...
        for params in theirs["sub_terrains"].values()
    )
    assert sum(
        params["proportion"] for params in theirs["sub_terrains"].values()
    ) == pytest.approx(1.0)

    broken = dict(theirs, num_cols=999)
    assert broken["num_cols"] != mj_ref.terrain_recipe()["num_cols"], (
        "the cache must not be shared by reference"
    )
...
def test_amp_symmetric_augmentation_matches_instinctmj(task) -> None:
    ...
    """
    source = mj_ref.motion_source()
    assert source["symmetric_augmentation_link_mapping"] == [
        0,
        1,
        3,
        2,
    ...
    assert mirror is not None, (
        "parkour declares no mirror augmentation; InstinctMJ mirrors half its resets"
    )
...
def test_ppo_runner_fields_match_instinctmj(our_agent) -> None:
    ...
    assert (
        our_agent["num_steps_per_env"]
        == reference["G1ParkourPPORunnerCfg.num_steps_per_env"]
    )
...
def test_agent_normalizers_are_empty_like_both_references(our_agent) -> None:
    ...
    assert (
        our_agent["policy"]["encoder_configs"]["depth_encoder"][
            "takeout_input_components"
        ]
        is True
    )
...
def test_instinct_rl_normalizer_cfg_default_is_a_running_zscore_not_identity() -> None:
    ...
    norm = build_normalizer(
        input_shape=4, normalizer_class_name=dumped["class_name"], normalizer_kwargs={}
    )
    sample = torch.tensor([[2.0, 4.0, 6.0, 8.0], [4.0, 8.0, 12.0, 16.0]])
    out = norm(sample)
    assert not torch.allclose(out, sample)
    assert (out - sample).abs().max().item() == pytest.approx(
        15.002493858337402, abs=1e-5
    )
...
def test_documented_drifts_are_still_present(task, compiled) -> None:
    ...
    assert "parkour_motion_without_run_retargetted.npz" not in (
...
def _lag_partition(robot) -> set[frozenset[str]]:
    """Which joints share one lag draw, read from the explicit MJLab configs."""
    from instinctlab.assets.unitree_g1.mjlab import beyondmimic_delayed_actuator_cfgs

    class RecordedActuator:
        def __init__(self, **kwargs):
            self.__dict__.update(kwargs)

    by_period: dict[int, set[str]] = {}
    for actuator in beyondmimic_delayed_actuator_cfgs(RecordedActuator):
        names = {
            name
        ...
        by_period.setdefault(int(actuator.delay_update_period), set()).update(names)
...
def test_lag_partition_matches_instinctmj(task) -> None:
    ...
    theirs = {
        frozenset(
            name
            for name in joints
            if any(re.fullmatch(pattern, name) for pattern in group)
        )
        for group in mj_ref.delayed_actuator_lag_groups()
    }
    assert frozenset() not in theirs, (
        "a reference pattern group matched no joint of ours"
    )
    assert set().union(*theirs) == set(joints), sorted(
        set(joints) - set().union(*theirs)
    )
...
def test_isaac_and_mjlab_agree_on_the_motor_buses() -> None:
    ...
    Both native files state their own groups explicitly. Editing either one alone must
    ...
    table = next(
        ...
        and any(
            ...
            and target.id == "beyondmimic_g1_29dof_delayed_actuators"
            for target in node.targets
        )
    )
    assert isinstance(table, ast.Dict)
    isaac = set()
    for call in table.values:
        assert isinstance(call, ast.Call)
        patterns = ast.literal_eval(
            next(keyword.value for keyword in call.keywords if keyword.arg == "joint_names_expr")
        )
        isaac.add(
            frozenset(
                joint
                for joint in robot.joint_names
                if any(re.fullmatch(pattern, joint) for pattern in patterns)
            )
        )
    mjlab = _lag_partition(robot)
    assert isaac == mjlab
...
def test_depth_history_first_push_matches_instinctmj_sensor_buffer() -> None:
    ...
    assert torch.allclose(
        term._history[0], torch.full_like(term._history[0], processed)
    )
...
def test_every_extractor_has_a_caller() -> None:
    ...
    unused = [
        name
        for name in public
        if f"mj_ref.{name}" not in source and name != "available"
    ]
...
def test_mjlab_camera_hit_semantics_match_instinctmj(task, compiled) -> None:
    ...
    semantics = pinhole_camera_effective_semantics(
        task.scene.ray_caster("camera"), task.sim.profiles["mjlab"]
    )
...
def test_isaac_camera_hit_semantics_track_main(task) -> None:
    ...
    assert set(task.scene.ray_caster("camera").hit_bodies()) == set(G1_29DOF_LINKS)
    assert "include_geom_groups" not in mj_ref.sensor_cfgs()["camera"]
    assert (
        mj_ref.sensor_cfgs()["camera"]["cfg_class"] == "NoisyGroupedRayCasterCameraCfg"
    )
...
def test_deliberate_rows_are_still_present(task, compiled) -> None:
    ...
    assert column_maps["instinctmj_built_num_cols"] == len(
        column_maps["sub_terrain_names"]
    )
...
def test_the_prose_counts_the_drift_table() -> None:
    ...
    source = Path(__file__).read_text()
    counts = {
        int(match) for match in re.findall(r"len\(KNOWN_DRIFTS\) == (\d+)", source)
    }
