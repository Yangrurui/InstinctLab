...
_PARKOUR = registry.spec("Instinct-Parkour-Target-G1", _ROBOT)
PARKOUR_MOTION_LINKS = _PARKOUR.scene.motion_references[0].links
...
def test_parkour_turns_named_mirroring_on() -> None:
    ...
    sensor = parkour_target_g1(_ROBOT).scene.motion_reference("motion_reference")
...
def test_locomotion_tasks_do_not_open_mirroring() -> None:
    ...
    flat_robot = _ADAPTER.robot_spec(registry.asset_id("Instinct-Velocity-Flat-G1"))
    rough_robot = _ADAPTER.robot_spec(registry.asset_id("Instinct-Velocity-Rough-G1"))
    assert flat_g1(flat_robot).scene.motion_references == ()
    assert rough_g1(rough_robot).scene.motion_references == ()
