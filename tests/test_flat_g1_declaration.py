...
TASK_OWNED_ACTUATOR_TERMS = {"dof_acc_l2", "dof_torques_l2", "feet_slide"}
"""Portable algorithms whose engine-neutral reads are owned by the task family."""
...
def test_actuator_and_contact_rewards_are_task_owned_portable_functions(task) -> None:
    by_kind = {name for name, term in task.mdp.rewards["rewards"].items() if term.kind is not None}
    assert by_kind == set()
    assert all(task.mdp.rewards["rewards"][name].func is not None for name in TASK_OWNED_ACTUATOR_TERMS)
...
def test_the_backend_reports_that_it_can_run_this_task(task) -> None:
    ...
    report = IsaacSimAdapter().contract_report(
        task_spec("Instinct-Velocity-Flat-G1", "isaacsim")
    )
