...
from instinctlab.compat.denylist import DENYLIST, PortabilityError
...
def test_force_semantics_are_denylisted() -> None:
    entry = DENYLIST["net_forces_w"]
    assert "normal" in entry.per_engine["isaacsim"]
    assert "contact frame" in entry.per_engine["mjlab"]
    assert "in_contact" in entry.resolution
