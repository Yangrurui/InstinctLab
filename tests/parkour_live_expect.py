...
from tests.live_device import (
    resolve_live_device,
)  # re-exported: existing imports resolve it here
...
def driven_joint_names(action_term) -> list[str]:
    return list(
        getattr(action_term, "target_names", getattr(action_term, "_joint_names", []))
    )


def assert_parkour_live_invariants(
    ...
    assert not missing, (
        f"command metrics missing {missing}; have {sorted(command.metrics)}"
    )
    ...
    assert built_cols == len(expected_columns), (
...
def assert_terrain_type_shares(
    terrain_types, column_names, declared: dict[str, float], *, num_envs: int
) -> None:
...
SCANNER_PITCH_JOINTS: tuple[str, ...] = (
    "left_ankle_pitch_joint",
    "right_ankle_pitch_joint",
)
SCANNER_ROLL_JOINTS: tuple[str, ...] = (
...
def assert_foot_scanner_shape(env) -> None:
    ...
    for name in SCANNER_NAMES:
        ...
        assert tuple(hits.shape) == (env.num_envs, SCANNER_RAYS, 3), (
            name,
            tuple(hits.shape),
        )
...
def assert_foot_scanner_uses_yaw_alignment(env, *, device: str) -> None:
    ...
    offset = torch.tensor(
        SCANNER_OFFSET, device=device, dtype=robot.data.body_link_pos_w.dtype
    )
...
def assert_foot_scanner_sees_some_ground(env) -> None:
    ...
    assert any(finite), (
        "every foot-scanner ray missed after reset; the caster is not hitting parkour terrain"
    )
...
def assert_foot_scanner_miss_is_positive_infinity(env, *, device: str) -> None:
    ...
    for name in SCANNER_NAMES:
        hits = ray_hits_w(env.scene.sensors[name])
        assert torch.isposinf(hits).all(), (
            f"{name}: miss must be +inf on every axis, finite={hits[torch.isfinite(hits)][:12].tolist()}"
        )
...
def assert_depth_first_policy_obs_is_primed(env) -> None:
    ...
    assert image is not None, (
        "policy group is concatenated; depth_image must stay a separate term"
    )
    assert tuple(image.shape[1:]) == (CAMERA_HISTORY_FRAMES, *CAMERA_CROP_HW), tuple(
        image.shape
    )
    newest = image[:, -1]
...
def assert_depth_camera_miss_is_positive_infinity(env, *, device: str) -> None:
    ...
    assert torch.isposinf(sky).all(), (
        f"sky-band miss must be +inf, finite={sky[torch.isfinite(sky)][:12].tolist()}"
    )
    finite = raw[torch.isfinite(raw)]
    if finite.numel():
        assert bool((finite > 0.0).all()) and bool((finite <= 2.5).all()), (
...
def assert_depth_camera_uses_base_alignment(env, *, device: str) -> None:
    ...
    if hasattr(robot, "write_root_link_velocity_to_sim"):
        robot.write_root_link_velocity_to_sim(
            torch.zeros(env.num_envs, 6, device=device), env_ids=env_ids
        )
    q = robot.data.default_joint_pos.clone()
    robot.write_joint_state_to_sim(q, torch.zeros_like(q), env_ids=env_ids)
    env.scene.write_data_to_sim()
    ...
    base_pos, base_quat = camera_pose_for_alignment(
        torso_pos, torso_quat, offset, rot, "base"
    )
    yaw_pos, yaw_quat = camera_pose_for_alignment(
        torso_pos, torso_quat, offset, rot, "yaw"
    )
    slide = (base_pos - yaw_pos).norm(dim=-1)
    assert bool((slide > CAMERA_YAW_SEP_M).all()), (
    ...
    assert bool((err_base < CAMERA_ALIGN_TOL_M).all()), (
    ...
    x = torch.tensor(
        [1.0, 0.0, 0.0], device=device, dtype=measured_quat.dtype
    ).expand_as(measured_pos)
...
def assert_rewards_finite_and_alive(env, *, steps: int, device: str) -> None:
    ...
    for step in range(steps):
        ...
        if float(reward.abs().sum()) > 0.0:
            saw_nonzero = True
        if step == 0:
            assert not bool((terminated | truncated).all()), (
                "every env done on the first step"
            )
...
def assert_policy_joint_dfs_runtime_semantics(env, spec, *, device: str) -> None:
    ...
    assert tuple(native) == G1_29DOF_ISAAC_BFS_JOINT_NAMES, (
        "recorded Isaac native joint order is stale"
    )
    assert list(spec.robot.joint_names)[0] == "waist_pitch_joint"
    assert native.index("waist_pitch_joint") != 0, (
        "PhysX native order must stay BFS, not DFS"
    )
    ...
    action_term_names = [
        name for name in ("actions", "last_action") if name in policy_term_names
    ]
    ...
    term_cfgs = {
        name: _policy_obs_term_cfg(env, name)
        for name in (*joint_terms, action_term_name)
    }
    for name in joint_terms:
        ...
        assert ids == expected_ids, (
            f"policy/{name} joint_ids != name-gathered DFS indices"
        )
    ...
    history_lengths = {
        name: int(getattr(term_cfgs[name], "history_length", 0) or 0)
        for name in expected_by_term
    }
...
def assert_depth_encoder_is_fed(env, spec, wrapper, agent_cfg) -> dict[str, int]:
    ...
    assert not torch.allclose(out, out_zero, atol=1e-5), (
        "encoder ignored a depth wipe; it is not being fed"
    )
    del extras
...
def assert_volume_points_shape(env, spec) -> None:
    ...
    assert tuple(offset.shape) == (env.num_envs, 2, VOLUME_POINTS_PER_BODY, 3), tuple(
        offset.shape
    )
...
def summarize_virtual_obstacles(terrain, *, xy_aabb=None) -> dict[str, object]:
    ...
    if xy_aabb is not None:
        ...
        summary["tile_centroid"] = (
            kept_mid.mean(dim=0).tolist() if kept.numel() else None
        )
...
def assert_volume_points_registered(env) -> dict[str, object]:
    ...
    tile = summarize_virtual_obstacles(
        terrain, xy_aabb=pyramid_stairs_tile_aabb(terrain)
    )
    assert int(full["count"]) > 0, full
    print(
        f"[volume-points] registered {count} cylinders; terrain {full}; pyramid_stairs tile {tile}"
    )
...
def assert_terrain_generated_cylinder_penetration(
    ...
    radius = float(
        getattr(
            getattr(obstacle, "cfg", None), "cylinder_radius", TERRAIN_CYLINDER_RADIUS
        )
    )
    lengths = torch.linalg.vector_norm(edges[:, 3:6] - edges[:, :3], dim=-1)
    usable = lengths > 0.2
    ...
    fallback = torch.linalg.cross(
        unit, torch.tensor((1.0, 0.0, 0.0), device=device).expand_as(unit)
    )
    perp = torch.where(
        torch.linalg.vector_norm(perp, dim=-1, keepdim=True) > 1e-4, perp, fallback
    )
    ...
    hand = _hand_cylinder_offset(
        sensor.data.points_pos_w[0].reshape(-1, 3), starts[0], ends[0], radius
    ).reshape_as(sensor.data.points_pos_w[0])
    ...
    def __init__(self, start, end, radius: float, device: str):
        ...
        self.cylinders = CylinderSpatialGrid(
            cylinders=np.array(
                [
                    [
                        *start.detach().cpu().tolist(),
                        *end.detach().cpu().tolist(),
                        radius,
                    ]
                ],
                dtype=np.float32,
...
def assert_known_volume_penetration(env, spec, *, device: str) -> dict[str, float]:
    ...
    start = sample + torch.tensor(
        (VOLUME_KNOWN_AXIS_OFFSET, -0.5, 0.0), device=device, dtype=sample.dtype
    )
    end = sample + torch.tensor(
        (VOLUME_KNOWN_AXIS_OFFSET, 0.5, 0.0), device=device, dtype=sample.dtype
    )
    ...
    try:
        ...
        assert bool((offset_err < 5e-3).all()), (
        ...
        assert sample_depth == pytest.approx(
            VOLUME_CYLINDER_RADIUS - VOLUME_KNOWN_AXIS_OFFSET, abs=2e-3
        )
...
def assert_known_volume_spin_velocity(env, *, device: str) -> dict[str, float]:
    ...
    if com_reader is not None:
        try:
            ...
            assert float((reported[0] - engine_com[0]).norm()) > 0.02, (
    ...
    hand = v_link.unsqueeze(1) + torch.linalg.cross(
        omega.unsqueeze(1).expand_as(r), r, dim=-1
    )
    ...
    mixed_hand = v_com.unsqueeze(1) + torch.linalg.cross(
        omega.unsqueeze(1).expand_as(r), r, dim=-1
    )
    mixed_err = (measured - mixed_hand).norm(dim=-1)
    assert float(mixed_err[0].mean()) > float(err[0].mean()) + 0.01, (
