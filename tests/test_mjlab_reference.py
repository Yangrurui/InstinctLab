...
def spec():
    return task_spec("Instinct-Velocity-Flat-G1")
...
def test_reference_specific_rewards_are_now_task_owned_portable_functions(rewards):
    by_kind = {name for name, term in rewards.items() if term.kind is not None}
    assert by_kind == set()
    assert all(rewards[name].func is not None for name in ("feet_slide", "dof_acc_l2", "dof_torques_l2"))
    for name in by_kind:
...
def test_the_friction_range_is_the_collapse_the_reference_performs(spec):
    ...
    taking the lower low and the higher high. The task's MJLab event parameters hold that interval;
    an engine-global profile would make the locomotion recipe a hidden default for every task.
    """
    ...
    declared = spec.mdp.events["physics_material"].engine_params["mjlab"]
    assert declared["ranges"] == expected
    assert declared["operation"] == "abs"
    assert declared["shared_random"] is True
...
def test_the_contact_groups_are_the_reference_groups(spec):
    ...
    groups = {
        "feet_contact_forces": spec.mdp.rewards["rewards"]["feet_air_time"].params["sensor"],
        "base_contact_forces": spec.mdp.terminations["base_contact"].params["sensor"],
    }
