...
def _robot(backend: str = "mjlab") -> RobotSpec:
    return RobotSpec(
        ...
        assets=(BackendAsset(backend=backend, path=f"robot.{ 'urdf' if backend == 'isaacsim' else 'xml'}"),),
...
def test_scene_sensor_names_may_not_shadow_scene_entities_or_isaac_fields():
    ...
    task = _task(
        robot=_robot("isaacsim"),
        scene=SceneSpec(contact_sensors=(ContactSensorRef(name="lazy_sensor_update", elements="foot"),)),
    )
...
def test_a_task_is_materialized_with_exactly_one_engine_owned_robot_asset():
    _task(robot=_robot("mjlab")).validate()
    with pytest.raises(ValueError, match="exactly one engine-owned robot asset"):
        _task(robot=replace(_robot(), assets=())).validate()
    both = replace(
        _robot(),
        assets=(
