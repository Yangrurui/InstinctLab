...
from instinctlab.engines.motion_reference import clip_frame, make_buffers
from instinctlab.mdp.amp import (
    AMP_TERM_ORDER,
    GRAVITY_DOWN_W,
...
def _catalog_defaults() -> torch.Tensor:
    return torch.tensor([G1_29DOF_DEFAULT_JOINT_POS[name] for name in G1_29DOF_DFS_JOINT_NAMES])
...
def _clip_tensors():
    ...
    joint_pos = _catalog_defaults().unsqueeze(0) + torch.linspace(-0.2, 0.2, n_joints).unsqueeze(0)
...
def test_policy_builder_on_clip_state_matches_reference_branch() -> None:
    ...
    quat, lin_w, ang_w, joint_pos, joint_vel, defaults, default_vel, gravity = _clip_tensors()
    like = robot_like_from_clip(quat, lin_w, ang_w, joint_pos, joint_vel, defaults, default_vel, gravity)
...
def test_amp_joints_are_canonical_names_not_bfs_or_clip_order() -> None:
    ...
    remapped, index_map = remap_by_name(raw["joint_pos"][:1], source, canonical, what="joint")
    ...
    assert remapped[0, waist_dst].item() == pytest.approx(raw["joint_pos"][0, waist_src].item())
...
def test_depth_encoder_component_names_resolve_against_the_declared_term() -> None:
    ...
    broken = {**encoder, "depth_encoder": {**encoder["depth_encoder"], "component_names": ["not_a_term"]}}
...
def test_depth_encoder_output_moves_when_the_image_moves() -> None:
    ...
    layer = ParallelLayer(segments, G1ParkourTargetPPORunnerCfg().to_dict()["policy"]["encoder_configs"])
    layer.eval()
    with torch.no_grad():
        out = layer(flat)
    ...
    assert not torch.allclose(out[:, latent_slice], out_no_proprio[:, latent_slice] * 0, atol=1e-5)
