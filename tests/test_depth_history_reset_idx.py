...
def _assert_depth_stack_primed(image: torch.Tensor, env_idx: int, *, tol: float = 1e-5) -> None:
    stack = image[env_idx]
    newest = stack[-1]
...
def test_mjlab_reset_idx_clears_subset_before_first_post_reset_obs() -> None:
    ...
    compiled = MjlabAdapter().compile(parkour_target_g1(), num_envs=8, device="cpu")
...
@pytest.mark.mjlab
@pytest.mark.skipif(not torch.cuda.is_available(), reason="mjlab parkour stepping needs a GPU")
def test_mjlab_auto_reset_256_subset_clears_depth_via_reset_idx() -> None:
    ...
    compiled = MjlabAdapter().compile(parkour_target_g1(), num_envs=256, device=device)
...
def test_isaac_reset_idx_clears_subset_before_first_post_reset_obs() -> None:
    ...
    compiled = IsaacSimAdapter().compile(parkour_target_g1(), num_envs=4, device=device)
    compiled.env_cfg.seed = 42
    env = compiled.make_env()
    try:
        env.reset()
...
def test_isaac_step_auto_reset_clears_depth_via_reset_idx() -> None:
    ...
    compiled = IsaacSimAdapter().compile(parkour_target_g1(), num_envs=8, device=device)
