...
from instinctlab.tasks.parkour.mdp.observations import _gaussian_blur
...
def test_depth_gaussian_blur_uses_instinctmj_reflect_padding() -> None:
    ...
    reflected = F.conv2d(
        F.pad(image.unsqueeze(1), (1, 1, 1, 1), mode="reflect"), kernel
    ).squeeze(1)
    replicated = F.conv2d(
        F.pad(image.unsqueeze(1), (1, 1, 1, 1), mode="replicate"), kernel
    ).squeeze(1)
