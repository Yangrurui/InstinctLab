...
KNOWN_DRIFTS: dict[str, tuple[str, str, str]] = {
    ...
    "obs/base_lin_vel_frame": (
        ...
        "task-local base_lin_vel → root_link_lin_vel_b",
        (
...
def _load_main_agent() -> dict:
    shown = subprocess.run(
        ("git", "show", f"main:{MAIN_AGENT}"), cwd=REPO, capture_output=True, text=True
    )
...
def test_every_sim_param_parses_out_of_main() -> None:
    ...
    assert not unparsed, (
        f"{unparsed} did not parse out of main's ParkourEnvCfg.__post_init__"
    )
...
def test_reward_names_match_main(task) -> None:
    ...
    assert declared == reference, (
        f"only main {sorted(reference - declared)}; only new {sorted(declared - reference)}"
    )
...
def test_terminations_only_drop_documented_ones(task) -> None:
    ...
    assert dropped == DELIBERATE_OMISSIONS, (
        f"unexpected drops {dropped - DELIBERATE_OMISSIONS}"
    )
...
def test_policy_and_critic_observation_order_matches_main(task) -> None:
    assert tuple(task.mdp.observations["policy"].terms) == main_ref.observation_order(
        "policy"
    )
    assert tuple(task.mdp.observations["critic"].terms) == main_ref.observation_order(
        "critic"
    )
...
def test_amp_observation_order_matches_main(task) -> None:
    assert tuple(
        task.mdp.observations["amp_policy"].terms
    ) == main_ref.observation_order("amp_policy")
    assert tuple(
        task.mdp.observations["amp_reference"].terms
    ) == main_ref.observation_order("amp_reference")
...
def test_observation_scales_match_main(task, group) -> None:
    ...
    declared = {
        name: _declared_scale(term)
        for name, term in task.mdp.observations[group].terms.items()
    }
...
def test_observation_history_and_clip_match_main(task, group) -> None:
    ...
    declared_clip = {
        name: term.clip for name, term in task.mdp.observations[group].terms.items()
    }
...
def test_agent_shared_hyperparameters_match_main(our_agent, main_agent) -> None:
    ...
    differing = {
        key: (main_agent[key], our_agent[key])
        for key in shared
        if main_agent[key] != our_agent[key]
    }
...
def test_documented_drifts_are_still_present(task) -> None:
    ...
    from instinctlab.tasks.parkour.mdp.observations import base_lin_vel
    ...
    assert rewards["undesired_contacts"].kind is None
    assert rewards["undesired_contacts"].func is undesired_contacts_by_force
    ...
    for func in (rewards["track_lin_vel_xy_exp"].func, rewards["dont_wait"].func):
        ...
        assert "root_link_lin_vel_b" in body and ".root_lin_vel_b" not in body, (
            func.__name__
        )
    from instinctlab.tasks.parkour.mdp.commands import PoseVelocityCommand

    metrics = inspect.getsource(PoseVelocityCommand._update_metrics)
    ...
    assert (
        source.get("symmetric_augmentation_link_mapping") is not None
        or "symmetric_augmentation_link_mapping" in source
    )
    assert (
        task.scene.motion_reference("motion_reference").symmetric_augmentation
        is not None
    )
...
def test_dof_vel_limits_match_mains_actuator_table(task) -> None:
    ...
    ours = dict(
        zip(term.params["asset_cfg"].joints, term.params["limits"], strict=True)
    )
    theirs = main_ref.actuator_joint_velocity_limits(G1_29DOF_DFS_JOINT_NAMES)
    assert ours == theirs, {
        j: (ours[j], theirs[j]) for j in ours if ours[j] != theirs[j]
    }
    assert (
        term.params["soft_ratio"]
        == main_ref.reward_params()["dof_vel_limits"]["soft_ratio"]
        == 0.9
    )
...
def test_the_actuator_limit_reader_refuses_a_table_it_cannot_expand() -> None:
    ...
    with pytest.raises(LookupError, match="without a velocity limit"):
        main_ref.actuator_joint_velocity_limits(
            [*G1_29DOF_DFS_JOINT_NAMES, "no_such_joint"]
        )
...
def test_critic_and_amp_lin_vel_use_the_hub_link_spelling(task) -> None:
    ...
    from instinctlab.tasks.parkour.mdp.amp import base_lin_vel_from_reference
    from instinctlab.tasks.parkour.mdp.observations import base_lin_vel

    assert task.mdp.observations["critic"].terms["base_lin_vel"].func is base_lin_vel
    assert (
        task.mdp.observations["amp_policy"].terms["base_lin_vel"].func is base_lin_vel
    )
    assert (
        task.mdp.observations["amp_reference"].terms["base_lin_vel"].func
        is base_lin_vel_from_reference
    )
    assert "root_link_lin_vel_b" in inspect.getsource(base_lin_vel)
    assert main_ref.observation_functions("critic")["base_lin_vel"] == "base_lin_vel"
    assert (
        main_ref.observation_functions("amp_policy")["base_lin_vel"] == "base_lin_vel"
    )
    assert (
        main_ref.observation_functions("amp_reference")["base_lin_vel"]
...
def test_main_amp_mirrors_half_the_reference_and_so_do_we(task) -> None:
    ...
    assert "parkour_motion_without_run.yaml" in str(
        amass["filtered_motion_selection_filepath"]
    )
    ...
    assert mirror is not None, (
        "parkour declares no mirror augmentation; main mirrors half its resets"
    )
...
def test_depth_delay_params_match_main(task) -> None:
    ...
    assert (
        tuple(ours["delayed_frame_ranges"])
        == tuple(theirs["delayed_frame_ranges"])
        == (0, 1)
    )
...
def test_camera_ray_alignment_config_differs_and_mjlab_honors_the_task_value(
    task,
) -> None:
    ...
    assert refuse_unhonored_ray_alignment(camera) is None
...
def test_wasabi_defaults_match_our_explicit_amp_keys(our_agent, main_agent) -> None:
    ...
    assert (
        our_agent.get(
            "policy.encoder_configs.depth_encoder.takeout_input_components", True
        )
        is True
    )


def test_runner_has_no_empirical_normalizer_on_either_side(
    our_agent, main_agent
) -> None:
...
def test_single_reward_group_keeps_num_rewards_one() -> None:
    ...
    task_id = "Instinct-Parkour-Target-G1"
    task = parkour_target_g1(robot_spec(registry.asset_id(task_id)))
...
def test_parkour_robot_matches_main_on_the_three_task_overrides(task) -> None:
    ...
    assert main_ref.effective_robot_actuators()["delayed"] is False
    assert task.robot.actuator_delay == (0, 2), (
        "kept on purpose; KNOWN_DRIFTS['actuation/delay']"
    )


@pytest.mark.isaacsim
def test_compiled_isaac_robot_matches_main_plant_and_keeps_documented_sim_drifts() -> (
    None
):
    ...
    assert {(cfg.min_delay, cfg.max_delay) for cfg in robot.actuators.values()} == {
        (0, 2)
    }
