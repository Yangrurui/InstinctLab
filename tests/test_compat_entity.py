...
def test_every_engine_registers_its_selectors() -> None:
    ...
    assert len(engines.ADAPTERS) >= 2, (
        f"only {sorted(engines.ADAPTERS)} adapters are known; the comparison below would be vacuous"
    )
    assert set(selector_kinds()) == set(engines.ADAPTERS), (
        f"registered engines {sorted(selector_kinds())} are not the known ones {sorted(engines.ADAPTERS)}; "
