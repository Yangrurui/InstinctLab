...
PACKAGE_INIT = (
    REPO / "source/instinctlab/instinctlab/tasks/parkour/config/g1/__init__.py"
)
AGENT = (
    REPO
    / "source/instinctlab/instinctlab/tasks/parkour/config/g1/agents/instinct_rl_amp_cfg.py"
)
TASK_OWNED_NATIVE_QUANTITIES = {
    "feet_slide",
    "dof_torques_l2",
    "dof_acc_l2",
    "energy",
...
"""Task formulas whose inputs preserve an intentional engine-native quantity."""

REQUIRED_NATIVE_QUANTITY = frozenset(TASK_OWNED_NATIVE_QUANTITIES)
...
def test_all_reward_formulas_are_owned_by_the_task(task) -> None:
    ...
    assert by_kind == set()
    assert by_func == set(REWARDS)
    for name in REQUIRED_NATIVE_QUANTITY:
...
def test_the_command_is_pose_velocity_and_required(task) -> None:
    ...
    assert term.func is PoseVelocityCommand
    assert term.level is Requirement.REQUIRED
...
def test_engine_params_only_carry_the_native_material_divergence(task) -> None:
    ...
    found = {
        key: dict(term.engine_params)
        for key, term in task.mdp.terms().items()
        if term.engine_params
    }
...
def test_volume_points_and_edge_cylinders_are_declared_and_required(task) -> None:
    ...
    assert term.level is Requirement.REQUIRED
    assert term.params["sensor"] == sensor
    ...
    assert event.func is register_virtual_obstacles
...
def test_the_depth_camera_is_declared_and_required(task) -> None:
    ...
    assert set(camera.hit_bodies()) == set(G1_29DOF_LINKS)
...
def test_the_joint_axis_is_pinned_to_the_canonical_order(task) -> None:
    ...
    for group, names in JOINT_TERMS.items():
        ...
        for name in names:
            selectors[f"observations.{group}.{name}"] = group_spec.terms[name].params[
                "asset_cfg"
            ]
    for path, ref in selectors.items():
        ...
        assert ref.preserve_order is True, (
            f"{path} names the joints but does not ask for their order"
        )
...
def test_joint_vel_limits_reads_the_catalog_in_canonical_order(task) -> None:
    ...
    assert term.params["limits"] == tuple(
        joint.velocity_limit for joint in task.robot.joint_properties
    )
    assert tuple(term.params["asset_cfg"].joints) == tuple(task.robot.joint_names)


def test_base_contact_force_threshold_is_task_owned(task) -> None:
    term = task.mdp.terminations["base_contact"]
    assert term.kind is None
    assert term.func is terminations.illegal_contact_by_force
    assert term.level is Requirement.REQUIRED
    assert term.params["sensor"].elements == ("torso_link",)
    assert term.params["threshold"] == 1.0


def test_undesired_contact_force_threshold_is_task_owned(task) -> None:
    term = task.mdp.rewards["rewards"]["undesired_contacts"]
    assert term.kind is None
    assert term.func is parkour_rewards.undesired_contacts_by_force
    assert term.level is Requirement.REQUIRED
    assert term.params["threshold"] == 1.0
...
def test_dataset_exhausted_matches_original_silent_reset(task) -> None:
    ...
    assert term.func is terminations.dataset_exhausted
...
def test_the_motion_reference_and_amp_groups_are_declared(task) -> None:
    ...
    from instinctlab.tasks.parkour.mdp.amp import AMP_TERM_ORDER
    sensor = task.scene.motion_reference("motion_reference")
    assert sensor.clip.endswith("parkour_motion_without_run_retargetted.npz")
    assert tuple(sensor.joints) == tuple(task.robot.joint_names)
    assert tuple(sensor.links) == (
        "pelvis",
        "torso_link",
    ...
    for name, term in task.mdp.observations["amp_reference"].terms.items():
        assert term.params["sensor"] == sensor, name
        assert tuple(term.params["asset_cfg"].joints) == tuple(task.robot.joint_names)
...
def test_the_agent_is_wasabi_moe_and_names_the_depth_component(task) -> None:
    ...
    assert dumped["policy"]["encoder_configs"]["depth_encoder"]["component_names"] == [
        "depth_image"
    ]
...
def test_the_declaration_imports_with_both_engines_blocked(monkeypatch) -> None:
    ...
    assert callable(module.parkour_target_g1)
...
def test_task_contract_materializes_for_both_backends() -> None:
    ...
    for adapter in (IsaacSimAdapter(), MjlabAdapter()):
        ...
        assert task.mdp.actions["joint_pos"].target.joints == robot.joint_names
...
def test_mjlab_compilation_preserves_declared_observation_history(task) -> None:
    ...
    for group_name, group_spec in task.mdp.observations.items():
        ...
        for term_name, term_spec in group_spec.terms.items():
            ...
            assert term_cfg.history_length == term_spec.history_length, (
...
def test_mjlab_compiles_every_kind_this_task_declares(task) -> None:
    ...
    command = task.mdp.commands["base_velocity"]
    assert list(command.params["random_velocity_terrain"]) == ["perlin_rough_stand"]
    assert set(command.params["velocity_ranges"]) == {
        "perlin_rough",
        "perlin_rough_stand",
        "square_gaps",
        "pyramid_stairs",
    ...
    assert "dense_boxes" not in command.params["velocity_ranges"]
    assert command.params["velocity_ranges"]["mesh_boxes"]["lin_vel_x"] == (0.45, 0.8)
    ...
    assert compiled.env_cfg.terminations["base_contact"].func is illegal_contact_by_force
    ...
    assert (
        events["register_virtual_obstacles"].params["sensor"].name
        == "leg_volume_points"
    )
    ...
    assert exhausted.func is terminations.dataset_exhausted
