...
**denylist：9 个同名不同义的语义陷阱**，误用必须报错，不得默认放行：
...
| `actuator_delay` | DelayedPD：物理步、闭区间，reset 抽一次保持到下回合 | BuiltinPD：同样是物理步、闭区间，但 `delay_update_period` 默认 0（每物理步重采样）。中枢是每回合每组抽一次；adapter 用超大 period + `delay_per_env_phase=False` + 各组不同 period |
...
| 轨 | 范围 | 关键交付物 |
|---|---|---|
| A · IR 与中枢 | `spec/` `compat/` `mdp/`；S1 / S2 / S3 | 带语义定义的 `vocab.py`、9 项 denylist、可移植 term 库 |
