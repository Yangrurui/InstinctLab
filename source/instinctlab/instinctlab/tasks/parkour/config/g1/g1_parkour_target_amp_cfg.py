"""Unified G1 Parkour task, ordered after InstinctMJ's native factory.
...
def parkour_g1_robot() -> RobotSpec:
    """G1 shoe model and InstinctMJ's delayed 29-DoF actuator contract."""
...
def _motion_reference(robot: RobotSpec) -> MotionReferenceRef:
...
def _observations(joints: EntityRef, motion_reference: MotionReferenceRef) -> dict[str, ObsGroupSpec]:
    return {
...
def parkour_target_g1() -> TaskSpec:
    """Build the unified training task without mutating a locomotion base config."""
    robot = parkour_g1_robot()
    joints = _canonical_joints(robot)
    motion_reference = _motion_reference(robot)

    return TaskSpec(
        task_id=TASK_ID,
        robot=robot,
        scene=_scene(motion_reference),
        sim=SimSpec(
            physics_dt=0.005,
            decimation=4,
        ...
        mdp=MdpSpec(
            commands=_commands(),
            observations=_observations(joints, motion_reference),
            actions=_actions(robot, joints),
            rewards={"rewards": _rewards(joints, _velocity_limits(robot))},
            curriculum=_curriculum(),
            terminations=_terminations(motion_reference),
            events=_events(),
        ),
        agent=AgentSpec(
