"""Robot-independent BeyondMimic task configuration."""
...
class BeyondMimicPlayEventsCfg:
    def __init__(self) -> None:
        self.physics_material = EventTermSpec(
            kind="randomize_friction",
            mode="startup",
...
class BeyondMimicRewardsCfg:
    def __init__(self, motion_reference: MotionReferenceRef) -> None:
        ...
        self.action_rate_l2 = RewardTermSpec(
            func=rewards.action_rate_l2, weight=-0.1, level=Requirement.REQUIRED
        )
        self.joint_limit = RewardTermSpec(
            func=rewards.joint_pos_limits, weight=-10.0, level=Requirement.REQUIRED
        )
...
class BeyondMimicEnvCfg:
    def __init__(
        ...
        self.sim = simulation
        policy = BeyondMimicPolicyObsCfg(robot)
        critic = BeyondMimicCriticObsCfg(robot, motion_reference)
        self.observations = {
            "policy": ObsGroupSpec(
        ...
        self.actions = dict(vars(BeyondMimicActionsCfg(robot)))
        self.commands = dict(vars(BeyondMimicCommandsCfg()))
        self.rewards = {"rewards": dict(vars(BeyondMimicRewardsCfg(motion_reference)))}
        self.events = events
        self.curriculum = curriculum
        self.terminations = dict(vars(BeyondMimicTerminationsCfg(motion_reference)))
