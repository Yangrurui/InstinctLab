...
def resolve_symmetric_augmentation(
    ...
    return ResolvedSymmetricAugmentation(
        joint_mapping=tuple(joint_index[spec.joint_swaps[name]] for name in joints),
        joint_signs=tuple(int(spec.joint_signs[name]) for name in joints),
        link_mapping=tuple(link_index[spec.link_swaps[name]] for name in links),
        joint_names=joints,
        link_names=links,
    )
...
class MotionReferenceRuntime:
    ...
    def advance(self, dt: float) -> torch.Tensor:
        ...
        if due.numel() == 0:
            return due
        self.refresh(due)
        self.last_update[due] = self.buffers.timestamp[due]
        return due
