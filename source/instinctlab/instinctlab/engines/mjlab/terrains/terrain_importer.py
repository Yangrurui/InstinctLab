...
class TerrainImporter(TerrainImporterBase):
    def __init__(self, cfg: TerrainImporterCfg, device: str):
        ...
        # building terrain spec manually in this terrain importer. ``Entity.__init__``
        # is not called -- it would rebuild the spec -- so every field it sets has
        # to be restated here. ``_variant_metadata`` is the one that fails first:
        # ``Scene.collect_variant_info`` reads it on every entity, including terrain.
        self._actuators = []
    ...
    @property
    def virtual_obstacles(self) -> dict[str, object]:
