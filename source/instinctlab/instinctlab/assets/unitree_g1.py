"""Isaac Lab ``ArticulationCfg`` views of the Unitree G1 catalog.

Physical constants and the canonical ``RobotSpec`` live in ``unitree_g1_spec``; this module only
binds those values to Isaac Lab spawn and actuator configs. It keeps the module name the release
published these configs under, so the parkour, shadowing and locomotion tasks that import them reach
the definitions directly rather than through a forwarding layer.
"""
...
from instinctlab.assets.unitree_g1_spec import (
    ARMATURE_4010,
    ARMATURE_5020,
    ARMATURE_7520_14,
    ARMATURE_7520_22,
...
_G1_SPEC = make_g1_29dof_robot_spec()
_G1_ISAAC_ASSET = _G1_SPEC.asset_for("isaacsim")

G1_29DOF_TORSOBASE_POPSICLE_CFG = ArticulationCfg(
    spawn=sim_utils.UrdfFileCfg(
        fix_base=_G1_ISAAC_ASSET.import_options["fix_base"],
        replace_cylinders_with_capsules=_G1_ISAAC_ASSET.import_options["replace_cylinders_with_capsules"],
        merge_fixed_joints=_G1_ISAAC_ASSET.import_options["merge_fixed_joints"],
        asset_path=_G1_ISAAC_ASSET.path,
        activate_contact_sensors=True,
    ...
    soft_joint_pos_limit_factor=_G1_SPEC.soft_joint_pos_limit_factor,
