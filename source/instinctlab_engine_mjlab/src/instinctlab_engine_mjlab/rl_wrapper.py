...
Ported from InstinctMJ's ``instinct_mj/rl/vecenv_wrapper.py``, which is the reference for how
mjlab locomotion training was run, so that the same rollout bookkeeping applies here.

The wrapper exists because ``instinct_rl``'s ``VecEnv`` contract is not the Gym contract: it wants
...
from instinctlab_engine.rl.termination_log import PortableTerminationLogger

if TYPE_CHECKING:
    from mjlab.envs import ManagerBasedRlEnv, ManagerBasedRlEnvCfg


class MjlabVecEnvWrapper(VecEnv):
    """Wrap mjlab's ``ManagerBasedRlEnv`` to match the ``instinct_rl`` VecEnv contract."""
    ...
    def get_obs_segments(self, group_name: str = "policy") -> dict[str, tuple[int, ...]]:
        ...
        return dict(zip(active_terms, term_dims, strict=False))
    ...
    def get_obs_format(self) -> dict[str, dict[str, tuple[int, ...]]]:
        ...
        for exposed_name, source_group in self._group_map.items():
            obs_format[exposed_name] = dict(zip(active_terms[source_group], term_dims[source_group], strict=False))

        for group_name, group_term_names in active_terms.items():
            if group_name in obs_format:
                continue
            obs_format[group_name] = dict(zip(group_term_names, term_dims[group_name], strict=False))
    ...
    def step(self, actions: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        # A grouped reward manager returns dict[str, Tensor]; stack into (num_envs, num_rewards).
    ...
    def _stabilize_log(self, extras: dict) -> None:
        """Keep ``extras['log']`` keys stable across steps for the logger."""
    ...
    def _flatten_group(self, *, group_name: str, group_obs: torch.Tensor | dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        return torch.cat([group_obs[name].flatten(start_dim=1) for name in active_terms], dim=1)
